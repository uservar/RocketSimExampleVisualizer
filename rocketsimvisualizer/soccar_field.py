...
goals_v = np.concatenate([soccar_goal_vertices + [0, -5120, 0],
                          soccar_goal_vertices * [1, -1, 1] + [0, 5120, 0]])
...
# text_lines = [f"v {p[0]} {p[1]} {p[2]}\n" for p in soccar_field_v]
# text_lines += [f"f {p[0]} {p[1]} {p[2]}\n" for p in soccar_field_f + 1]

# with open("soccar_field.obj", "w") as file:
